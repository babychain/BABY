...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BABY);
    ...
    if (!settings.contains("nAnonymizeBabyAmount"))
        settings.setValue("nAnonymizeBabyAmount", 1000);

    nAnonymizeBabyAmount = settings.value("nAnonymizeBabyAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBabyAmount"))
        SoftSetArg("-anonymizebabyamount", settings.value("nAnonymizeBabyAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in baby.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBabyAmount:
            return QVariant(nAnonymizeBabyAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBabyAmount:
            nAnonymizeBabyAmount = value.toInt();
            settings.setValue("nAnonymizeBabyAmount", nAnonymizeBabyAmount);
            emit anonymizeBabyAmountChanged(nAnonymizeBabyAmount);
            break;
        case CoinControlFeatures:
