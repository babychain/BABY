...
std::list<std::string> ZBABYControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBABYControlDialog::listMints;

ZBABYControlDialog::ZBABYControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBABYControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBABYControlDialog::~ZBABYControlDialog()
{
    delete ui;
}

void ZBABYControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBABYControlDialog::updateList()
{
...
void ZBABYControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBABYControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBABY_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBABYControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBABYControlDialog::GetSelectedMints()
{
...
void ZBABYControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
